...
import time
import json
from config import SecretId, SecretKey, CLB_ID

# 克隆CLB节点


def clone_load_balancer(clb_id):
...
def main():
    request_id = clone_load_balancer(CLB_ID)
    clone_clb_id = ""
    # 根据克隆CLB返回的request_id, 间隔1s轮询任务结果
    while True and request_id != "":
        task_data_obj = get_task_result(request_id)
        if "LoadBalancerIds" in task_data_obj and task_data_obj["LoadBalancerIds"] is not None:
            clone_clb_id = task_data_obj["LoadBalancerIds"][0]
            break
        time.sleep(1)
    print("backup clb id is %s" % clone_clb_id)
